...
fn main() {
    ...
    plt
        ...
        .set_style(PlotStyle::Nature)
        .set_xlabel(r"$\phi_c$")
        .set_ylabel(r"$\Delta V_{(1)}(\phi_c, T)$")
        .set_ylim((-1e6, 3e7))
        .set_dpi(600)
        .tight_layout()
...
pub fn m_h(phi_c: f64) -> f64 {
    (-MH.powi(2) + 3f64 * LAMBDAC * phi_c.powi(2)).abs().sqrt()
}

pub fn m_h2(phi_c: f64) -> f64 {
    -MH.powi(2) + 3f64 * LAMBDAC * phi_c.powi(2)
}
...
pub fn V1loop(phi_c: f64) -> f64 {
    ...
    1f64 / (64f64 * PI.powi(2)) * (
        ...
        + NH as f64 * m_h.powi(4) * ((m_h.powi(2) / MH.powi(2)).ln() - CH)
